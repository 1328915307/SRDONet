...
def train(args, train_dataset, AudioSpecFeat, validate_dataset, CE_weights, train_idx):
    ...
    optim_scheduler1 = torch.optim.lr_scheduler.StepLR(optimizer=optimizer1, step_size=30, gamma=0.95)
...
def trainDraw(args, train_dataset, AudioSpecFeat, validate_dataset, train_idx):
    ...
    RETURNED_AudioSpecFeat = None
    RETURN_BACKBONE = None
    RETURN_ENCODER = None
    best_epoch = 0
    # RETURN_CLASSFIER = None
    patience, Length, Plot_loss, best_val_acc = 10, len(train_dataset), [], 0
    start_time = time.time()
    for epoch in range(args.num_epochs):
        Loop = tqdm(enumerate(train_dataset), total=Length, colour='blue', leave=True)
        ...
        LossList, running_loss = [], 0.0
        for batch_idx, batch_data in Loop:
            train_batchSamples, train_batchLabels = batch_data['input1'], batch_data['label1']
            outputs = AudioSpecFeat(train_batchSamples.type(torch.FloatTensor).cuda())
            one_hot_labels = Variable(torch.zeros(args.batch_size, 2).scatter_(1, train_batchLabels.view(-1, 1), 1).cuda())
            Loss_classification = criterion(outputs, one_hot_labels)
            # epoch 要增多，保证学习率不下降太多。以分类loss为主，进一步优化参数设置。
            TotalLoss = Loss_classification
            # print(sort_Loss*100)
            AudioSpecFeat.zero_grad()
            TotalLoss.backward()
            optimizer1.step()
        ...
        if validate_dataset is not None:
            ...
            if val_result['acc'] >= best_val_acc:
                RETURNED_AudioSpecFeat = copy.deepcopy(AudioSpecFeat)
                RETURN_BACKBONE = copy.deepcopy(AudioSpecFeat.backbone)
                RETURN_ENCODER = copy.deepcopy(AudioSpecFeat.encoder)
                best_epoch = epoch
                best_val_acc = val_result['acc']
    ...
    MODEL = AudioSpecFeat
    print('Running time is :', time.time()-start_time)
    print('best acc epoch is' + str(best_epoch))
    return RETURN_MODEL, MODEL, RETURN_BACKBONE, RETURN_ENCODER, best_epoch
...
def evaluate(dataset, MODEL, toPersonList):
    ...
    with torch.no_grad():
        ...
        for m, data_test in enumerate(dataset):
            ...
            ypred, _,filter_out,attn = AudioSpecFeat(V_data.type(torch.FloatTensor).cuda())
...
def evaluate2(dataset, MODEL, toPersonList, ori_test_dataList):
    AudioSpecFeat = MODEL.eval()
    with torch.no_grad():
        target, Labels_, probs = [], [], []
        total_rewards = 0
        ...
        save_test_wav(toPersonList, ori_test_dataList, target, Labels_)
...
def save_test_wav(toPersonList,ori_test_dataList,target_, Labels_):
    Path = "./testData_oriwav"
    for idx, per in enumerate(toPersonList):
        if idx == 0:
            start = 0
        end = start + per
        personfilename = "person"+str(idx)
        target_split = target_[start:end]
        labels_split = Labels_[start:end]
        for pid,(target,label) in enumerate(zip(target_split,labels_split)):
            if target == label:
                savePath = os.path.join(os.path.join(os.path.join(Path,personfilename),"TruePre"),"patch"+str(pid)+".wav")
            else:
                savePath = os.path.join(os.path.join(os.path.join(Path,personfilename),"FalsePre"),"patch"+str(pid)+".wav")

            np2wav(ori_test_dataList[idx][pid],savePath)
        start = end
...
def evaluate_val(dataset, MODEL):
    ...
    with torch.no_grad():
        target, Labels_ = [], []
        for m, data_test in enumerate(dataset):
            V_data = Variable(data_test['input1'].to(torch.float32)).cuda()
            true_labels = data_test['label1'].long().numpy()
...
def evaluate3(dataset, MODEL, toPersonList):
    AudioSpecFeat = MODEL.eval()
    with torch.no_grad():
        target, Labels_, probs = [], [], []
        total_rewards = 0
        total_labels = 0
        for m, data_test in enumerate(dataset):
            V_data = Variable(data_test['input1'].to(torch.float32), requires_grad=False).cuda()
            true_labels = data_test['label1'].long().numpy()
            ypred= AudioSpecFeat(V_data.type(torch.FloatTensor).cuda())
            _, predict_labels = torch.max(ypred, dim=1)
            preds = predict_labels.cpu().data.numpy()
