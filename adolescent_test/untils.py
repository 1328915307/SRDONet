...
from adolescent_test.model_main import melBanks
...
def total_dataloader_DA(args, Total_Data, Total_label, STflag):
    if STflag == 0: # 0代表源域，1代表目标域
    ...
    for train_index, test_index in kf.split(non_severe_depressionList, labelList):
        ...
        # train_dataset =
        train_labels_dataset = torch.tensor(train_labels_dataset)
        train_corpus_label = torch.empty_like(train_labels_dataset, dtype=torch.int64)
        train_corpus_label.fill_(STflag)
        # Test
        # test_dataset = datafft(np.array(test_dataset), args)
        test_labels_dataset = torch.tensor(test_labels_dataset)
        test_corpus_label = torch.empty_like(test_labels_dataset, dtype=torch.int64)
        test_corpus_label.fill_(STflag)
        # Val
        # val_dataset = datafft(np.array(val_dataset), args)
        val_labels_dataset = torch.tensor(val_labels_dataset)
        val_corpus_label = torch.empty_like(val_labels_dataset, dtype=torch.int64)
        val_corpus_label.fill_(STflag)

        train_dataset = datasets_DA(train_dataset, train_labels_dataset, train_corpus_label)
        test_dataset = datasets_DA(test_dataset, test_labels_dataset, test_corpus_label)
        val_dataset = datasets_DA(val_dataset, val_labels_dataset, val_corpus_label)
...
def total_dataloader2(args, Total_Data, Total_label):
    zip_list = list(zip(Total_Data, Total_label))
    ...
    for train_index, test_index in kf.split(non_severe_depressionList, labelList):
        X_train, X_test = non_severe_depressionList[train_index], non_severe_depressionList[test_index]
        y_train, y_test = labelList[train_index], labelList[test_index]
        tv_folder = StratifiedKFold(n_splits=5, random_state=1, shuffle=True).split(X_train, y_train)
        # 划分验证集
        for t_idx, v_idx in tv_folder:
        ...
        for idx, data in enumerate(X_test):
            if len(data) / 16000 > seg_level:
                ...
                test_dataset.extend(data_temp)
                demo_per.append(data_temp.shape[0])
                ori_test_data.append(data_temp)
        toPersonList.append(demo_per)
        toSex.append(test_sex_dataset)
        ori_test_datalist.append(ori_test_data)

        # Train
        train_dataset = datafft(np.array(train_dataset), args)#64 640
        train_labels_dataset = torch.tensor(train_labels_dataset)
        # Test
        test_dataset = datafft(np.array(test_dataset), args)
        test_labels_dataset = torch.tensor(test_labels_dataset)
    ...
    return trainList_dataloader, testList_dataloader, valList_dataloader, toPersonList, ori_test_datalist
...
class datasets_DA(Dataset):
    def __init__(self, x, label,corpus):
        self.labels = label
        self.x = x
        self.corpus_label = corpus

    def __getitem__(self, idx):
        return_dic = {'x': self.x[idx],
                      'label': self.labels[idx],
                      'corpus':self.corpus_label[idx]}
...
def datafft(signal, args):
    ...
    stft = torch.stft(input=torch.from_numpy(signal).float(), n_fft=NFFT, window=window, hop_length=hop_length,
                      win_length=win_length, normalized=True, center=False, return_complex=True)#513 640
...
def plot_mel_spectrogram(filter_banks, sample_rate=16000, hop_length=160):
    # 转换为numpy数组
    filter_banks = filter_banks.cpu().detach().numpy()
    # # 假设使用第一个批次的数据进行绘图
    # filter_banks = filter_banks[0]

    # 计算时间轴和频率轴
    time_axis = np.linspace(0, filter_banks.shape[1] * hop_length / sample_rate, num=filter_banks.shape[1])
    mel_axis = np.linspace(0, sample_rate / 2, num=filter_banks.shape[0])

    plt.figure(figsize=(10, 4))
    plt.imshow(filter_banks, aspect='auto', origin='lower',
               extent=[time_axis.min(), time_axis.max(), mel_axis.min(), mel_axis.max()])
...
def dataloaderEATD(args, Total_Data, Total_label):
    ...
    Scaler = StandardScaler()
    # waveList_NC = ['positive_out.wav', 'neutral_out.wav']
    # waveList_MDD = ['neutral_out.wav', 'negative_out.wav']
    waveList_NC = ['positive_out.wav']
    waveList_MDD = ['negative_out.wav','negative.wav']
    ...
    for train_index, test_index in kf.split(non_severe_depressionList, labelList):
        ...
        for idx, data in enumerate(X_train):
            ...
            if y_train[idx] == 1:
                ...
                #     if len(np.array(data_array[i])) / sr > seg_level:
                #         data_temp = data_segmentwd(data_array[i], int(16000 * seg_level), 16000 * stride)
                #         m = data_temp.shape[0]
                #         label = y_train[idx]
                #         train_labels_dataset.extend(label.repeat(m))
                #         train_dataset.extend(data_temp)

                x, y = temp_data[0], temp_data[1]  # 均分2份，近似的模拟[netrul,negative]的拼接
                # 生成所有可能的排列组合
                permutations = list(itertools.permutations([x, y]))
                # 对所有排列组合进行拼接
                data_array = [np.concatenate(p) for p in permutations]
                for i in range(len(data_array)):
                    if len(np.array(data_array[i])) / sr > seg_level:
                ...
                # data_array = []
                # [data_array.extend(p) for p in temp_data]
                # if len(np.array(data_array)) / sr > seg_level:
                #     data_temp = data_segmentwd(np.array(data_array), int(16000 * seg_level), 16000 * stride)
                #     m = data_temp.shape[0]
                #     label = y_train[idx]
                #     train_labels_dataset.extend(label.repeat(m))
                #     train_dataset.extend(data_temp)
                ...
                if len(np.array(concat_array)) / sr > seg_level:
                    data_temp = data_segmentwd(np.array(concat_array), int(16000 * seg_level), 16000 * stride)
                    m = data_temp.shape[0]
                    label = y_train[idx]
                    train_labels_dataset.extend(label.repeat(m))
                    train_dataset.extend(data_temp)
    ...
    return trainList_dataloader, testList_dataloader, testList_dataloader, toPersonList
...
def dataloader_DAICwoz(args):
    ...
    for idx, data in enumerate(val_split):
        if len(data) / 16000 > semgent_l:
            if val_labels[idx] == 0:
                ...
                data_temp = data_segmentDAIC(data, int(16000 * semgent_l), int(16000 * semgent_l), 'Interpretability')
            m, _ = data_temp.shape
    ...
    return trainList_dataloader, valList_dataloader, valList_dataloader_shuffle, toPersonList_Val
